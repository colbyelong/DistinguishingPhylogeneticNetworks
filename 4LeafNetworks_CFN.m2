--The following code computes all of the ideals for 4-leaf CFN k-cycle networks
--to verify the statement in Proposition 4.7.
...
L = 
{(A,A,A,A),(A,A,C,C),(A,C,A,C),(A,C,C,A),(C,A,A,C),(C,A,C,A),
 (C,C,A,A),(C,C,C,C)};
...
L2 = {a_0,a_1,b_0,b_1,c_0,c_1,d_0,d_1,
      e_0,e_1,f_0,f_1,g_0,g_1,h_0,h_1,
      l_0,l_1};

R = QQ[(L2|L1), MonomialOrder=> Eliminate 18];


--2-cycle (tree) Network Topology with split 01|23
...
----Permuting the leaf labels -------------------------------------------------

------------------------------------------------------------------------------------------
T = {a_0,a_1,b_0,b_1,c_0,c_1,d_0,d_1,
     e_0,e_1,f_0,f_1,g_0,g_1,h_0,h_1,
     l_0,l_1}
...
S01 = {q_(0,0,0,0),q_(0,0,1,1),q_(1,0,0,1),q_(1,0,1,0),q_(0,1,0,1),q_(0,1,1,0),q_(1,1,0,0),q_(1,1,1,1)};
s01 = map(R,R,matrix{(T|S01)})

--(02)
S02 ={q_(0,0,0,0),q_(1,0,0,1),q_(0,1,0,1),q_(1,1,0,0),q_(0,0,1,1),q_(1,0,1,0),q_(0,1,1,0),q_(1,1,1,1)};
s02 = map(R,R,matrix{(T|S02)})

--(03)

S03 = {q_(0,0,0,0),q_(1,0,1,0),q_(1,1,0,0),q_(0,1,1,0),q_(1,0,0,1),q_(0,0,1,1),q_(0,1,0,1),q_(1,1,1,1)};
s03 = map(R,R,matrix{(T|S03)})

--(12)
S12 = {q_(0,0,0,0),q_(0,1,0,1),q_(0,0,1,1),q_(0,1,1,0),q_(1,0,0,1),q_(1,1,0,0),q_(1,0,1,0),q_(1,1,1,1)};
s12 = map(R,R,matrix{(T|S12)})

--(13)
S13 = {q_(0,0,0,0),q_(0,1,1,0),q_(0,1,0,1),q_(0,0,1,1),q_(1,1,0,0),q_(1,0,1,0),q_(1,0,0,1),q_(1,1,1,1)};
s13 = map(R,R,matrix{(T|S13)})

--(23)
S23 =  {q_(0,0,0,0),q_(0,0,1,1),q_(0,1,1,0),q_(0,1,0,1),q_(1,0,1,0),q_(1,0,0,1),
q_(1,1,0,0),q_(1,1,1,1)};
...
--Determine all equivalence classes of k-cycle network ideals
...
--We see that there are 3 tree network ideals (V_1,V_2,V_3), 
--3 3-cycle network ideals (B_1,B_2,B_3),
--and 3 4-cycle network ideals (H_1,H_3,H_5).


for j1 from 1 to 3 do
  for j2 from 1 to 3 do if isSubset(B_j2,V_j1) then print(toString (v_j1,b_j2))
  
for j1 from 1 to 3 do
  for j2 in {1,3,5} do if isSubset(H_j2,B_j1) then print(toString (b_j1,h_j2))
     
--We see that the tree network ideals are actually equal to the 3-cycle network ideals
--and that each of these is contained in 2 of the 3 4-cycle network ideals
